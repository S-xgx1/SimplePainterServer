...
public class WordController(MainDateBase context, IMapper mapper) : ControllerBase
{
    ...
    [HttpGet("CreateTimeList")]
    public async Task<ActionResult<IReadOnlyList<WordCreateTimeDto>>> GetCreateTimeList(int id)
    {
        var data = await context.WordCreateTimes.Where(x => x.WordInfoId == id).OrderByDescending(x => x.DateTime)
                                .ToArrayAsync();
        return mapper.Map<WordCreateTime[], WordCreateTimeDto[]>(data);
    }
