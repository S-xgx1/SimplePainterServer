...
public class AutoProfile : Profile
{
    public AutoProfile()
    {
        CreateMap<Guess, GuessDto>().ConstructUsing(x => new(x.ID, x.ImageId, x.Word, x.UserID)).ReverseMap()
                                    .ConstructUsing(x => new(x.ImageId, x.Word, x.UserID));
        CreateMap<Guess, GuessDetail>().ConstructUsing(x => new(x.ID, x.ImageId, x.Word, x.UserID));
        CreateMap<ImageInfo, ImageInfoDto>().ConstructUsing(x => new(x.ID, x.WordId, x.UserID)).ReverseMap()
                                            .ConstructUsing(x => new(x.ID, x.WordId, x.UserID));
        CreateMap<UserInfo, UserInfoDto>()
           .ConstructUsing(x => new(x.ID, x.Name, x.Language, x.Sex, x.Age, x.Career, x.EducationLevel)).ReverseMap()
